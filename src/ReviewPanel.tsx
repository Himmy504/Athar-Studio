import { TRANSLATION_LANGUAGES, targetLanguage, changeTargetLanguage, type TranslationLanguage, defaultLanguageFont } from './languages';
    ...
    <div className="caption-list">
      ...
      filtered.map(s=>{
        ...
        return <article key={s.id} data-caption-id={s.id} aria-current={selectedId===s.id?'true':undefined} onFocusCapture={()=>onSelect(s.id)} onClick={()=>onSelect(s.id)} className={'caption-card '+(open?'expanded ':'')+(selectedId===s.id?'selected ':'')+(checked?'approved':'')}>
          ...
          <label className="caption-language-label"><span>{translation.name}</span><textarea aria-label={translation.name+' caption '+(i+1)} className="english-input" dir={translation.rtl?'rtl':'ltr'} lang={translation.code} style={{fontFamily:defaultLanguageFont(translation.code),unicodeBidi:'isolate'}} rows={Math.min(5,Math.max(1,s.english.split('\n').length))} value={s.english} placeholder={translation.name+' translation'} disabled={busy} onChange={e=>modify(s.id,v=>editSegment(v,{english:e.target.value}),'english-'+s.id)}/></label>
...
function SplitDialog({translation,segment:s,onClose,onSplit}:{translation:ReturnType<typeof targetLanguage>;segment:Segment;onClose:()=>void;onSplit:(at:number,a:number,e:number)=>void}){
  ...
  return <Modal title="Split this caption" subtitle="Click at the split point in each language, then set the matching time." onClose={onClose}>
    ...
    {s.english&&<><Field label={translation.name+' text split'}><textarea lang={translation.code} style={{fontFamily:defaultLanguageFont(translation.code)}} dir={translation.rtl?'rtl':'ltr'} readOnly value={s.english} onSelect={v=>setE(v.currentTarget.selectionStart)}/></Field><p className="split-preview" lang={translation.code} style={{fontFamily:defaultLanguageFont(translation.code)}} dir={translation.rtl?'rtl':'ltr'}>{s.english.slice(0,e)} <b>│</b> {s.english.slice(e)}</p></>}
